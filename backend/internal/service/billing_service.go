...
func (s *BillingService) Void(id uint64) (*model.Billing, error) {
	...
	if b.Status == constants.BillingStatusVoid {
		s.logger.Warn(constants.LogBillingVoidFailed, "billing_id", id, "status", b.Status)
		return nil, util.NewAppError(constants.CodeBillingStatusConflict, "Billing[id="+u64(id)+"] void failed: status="+b.Status)
	}
